  ...
  module Checkout
    class PaypalWalkthrough
      def self.run(client)
        ...
        # First up: a paypal payment start
        paypal_payment_method = order['payment_methods'].detect { |pm| pm['name'] == "Paypal Express Checkout" }

        response = client.post("/api/orders/#{order['number']}/paypal/start?" +
                               "return_url=#{URI.encode('http://localhost:3000/orders/1')}" +
                               "&cancel_url=#{URI.encode('http://localhost:3000/checkout/payment')}" +
                               "&payment_method_id=#{paypal_payment_method['id']}")

        if response.status == 200
          response_body = JSON.parse(response.body)
          # {"success"=>true, "redirect_url"=>"https://www.sandbox.paypal.com/webscr?cmd=_express-checkout&token=EC-6YB13058VG3277242&useraction=commit"}

          # redirect user to this url response_body["redirect_url"]

          client.succeeded "Paypal redirect url recieved."
        else
          client.failed "Paypal redirect url not recieved."
        end

        # after login to paypal token and payerID will be recieved on return_url
        # Example: return_url?token=EC-9H111010FP408652E&PayerID=BGWLU36AE2HF6

        # confirm payment by making api call
        response = client.post("/api/orders/#{order['number']}/paypal/confirm?" +
                               "paypal_token=EC-9H111010FP408652E" +
                               "&PayerID=BGWLU36AE2HF6}" +
                               "&payment_method_id=#{paypal_payment_method['id']}")

       if response.status == 200
         order = JSON.parse(response.body)
         client.succeeded "Paypal Payment details provided for the order."
       else
         client.failed "Paypal Payment details were not accepted for the order."
       end

       # Next two steps are remaining. Confirm and Complete which can be done as below
       # Currently commented out as we have used temporary token and payerID above.
       ...
       #     client.succeeded "Order transitioned to 'confirm'."
       #   else
       #     client.failed "Order did not transition to 'confirm'."
       #   end
       # else
       #   client.failed "Order could not transition to 'confirm'."
       # end
...
Examples.run(Examples::Checkout::PaypalWalkthrough)
